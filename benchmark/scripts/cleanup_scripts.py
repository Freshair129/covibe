...
master_logic = r"""
    ...
    function initHnswVisualizer() { console.log("Initializing HNSW Visualizer..."); }

    window.onload = () => { switchDomain('overview'); animateReactor(); loadBenchmarkData(); };
"""

# 2. Scrub all existing scripts
content = re.sub(r'<script>.*?</script>', '', content, flags=re.DOTALL)

# 3. Add back Tailwind and Cytoscape
header_scripts = r"""
  <!-- Cytoscape.js -->
...
"""
if '<!-- Cytoscape.js -->' not in content:
    content = content.replace('</head>', header_scripts + '\n</head>')

# 4. Inject master logic
content = content.replace('</body>', f'<script>{master_logic}</script>\n</body>')
...
print("Full Script Reset & Path Hardening Complete.")
