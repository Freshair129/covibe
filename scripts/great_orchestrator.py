...
async def run_local(model, prompt, num_ctx):
    ...
    # Smart Stream Buffer
    last_print_len = 0
    repetition_check_buffer = ""
    
    try:
        print(f"\n[{model}] INFERENCE START (8K+ Ctx):")
        with requests.post(url, json=payload, stream=True, timeout=300) as resp:
            for line in resp.iter_lines():
                if line:
                    ...
                    # --- HYBRID PRINT LOGIC ---
                    # 1. พ่นโค้ด 500 ตัวแรกให้เห็นสไตล์การเขียน
                    if len(full_response) < 500:
                        print(text, end="", flush=True)
                    # 2. หลังจากนั้นพ่นจุด "หัวใจ" ทุกๆ 100 ตัว
                    elif len(full_response) - last_print_len > 100:
                        print("❤", end="", flush=True)
                        last_print_len = len(full_response)
                        # 3. สุ่มพ่นบรรทัดล่าสุดออกมาเช็ค (สุ่มทุกๆ 1000 ตัว)
...
async def run_round(task_name, context_size):
    ...
    instruction = "STRICT RULES: Implement the requested logic in TypeScript. Output ONLY the core class/function and one simple usage. DO NOT repeat your reasoning after the code block. End your response with '### END'."
    
    if "Vitest" in task_name:
        task_desc = "Generate a comprehensive Vitest unit test for a utility function 'calculateSyncAction(clientTime, serverTime, drift)'. Rules: <250ms: ignore, 250-800ms: adjust_rate, >800ms: seek. Include edge cases."
    else:
        task_desc = "Implement a robust Circuit Breaker in TypeScript with states: CLOSED, OPEN, HALF_OPEN."
    ...
    for model in LOCAL_MODELS:
        ...
        # Thermal Check: Cool down if it was a heavy local run
...
async def main():
    ...
    # Round 1: 8K Standard
    all_benchmarks["8K"] = await run_round("L1_Standard", "8K")
    
    # Round 2: 16K Payload
    all_benchmarks["16K"] = await run_round("L1_Standard", "16K")

    # Round 3: 16K Vitest (The Real-world Task)
    all_benchmarks["16K_Vitest"] = await run_round("L3_Vitest_Gen", "16K")

    # Round 4: 32K Stress (Sushi Only)
    all_benchmarks["32K"] = await run_round("L2_Advanced", "32K")
